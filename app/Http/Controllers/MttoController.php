...
public function index()
{
    ...
    $data2 = DB::table('tbl_mantenimiento') ->get();
    $data = DB::table('mtto') ->get();
    return view('mantenimiento.index',compact('data','data2'));
}
...
public function consultaf(Request $request)
{
    ...
    $resultado= DB::select("SELECT * FROM mtto WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mes'");
    $resultadoii= DB::select("SELECT * FROM mtto WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mesii'");
