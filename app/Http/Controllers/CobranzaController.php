...
class CobranzaController extends Controller
{
    ...
    {
        $this->middleware(['auth', 'roles:cobranza,admin']);
    }

public function index()
{

    $data2 = DB::table('tbl_creditoycobranza') ->get();
    $data = DB::table('cobranza') ->get();
    return view('cobranza.index',compact('data', 'data2'));
}
public function consultaf(Request $request)
{
    $mes = $request->input('month');
    $mesii = $request->input('month2');
    $resultado= DB::select("SELECT * FROM cobranza WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mes'");
    $resultadoii= DB::select("SELECT * FROM cobranza WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mesii'");
