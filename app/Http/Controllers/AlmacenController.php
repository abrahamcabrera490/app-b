...
public function index()
{
    ...
    $data2 = DB::table('tbl_almacenes') ->get();
    $data = DB::table('almacen') ->get();
    return view('almacen.index', compact('data','data2'));
}
...
return redirect('/almacen');
    ...
    public function consultaf(Request $request)
    {
        ...
        $resultado= DB::select("SELECT * FROM almacen WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mes'");
        $resultadoii= DB::select("SELECT * FROM almacen WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mesii'");
