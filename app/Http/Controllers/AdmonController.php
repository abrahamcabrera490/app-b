...
public function index()
{
    ...
    $data = DB::table('admon') ->get();
    return view('administracion.index',compact('data','data2'));
}
 ...
 public function consultaf(Request $request)
    {
        ...
        $resultado= DB::select("SELECT * FROM admon WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mes'");
        $resultadoii= DB::select("SELECT * FROM admon WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mesii'");
...
public function prom(Request $request)
{
    ...
    $mespasado= DB::select("SELECT AVG(eventos) AS PROM1 FROM  admon WHERE MONTH(fecha)=  MONTH(NOW())-1");
    $fechaactual= DB::select("SELECT AVG(eventos) as PROM2 FROM  admon WHERE MONTH(fecha)=  MONTH(NOW())");
...
for($i = 0; $i<$inicio; $i++)
{
    $indicador = DB::table('admon')->insert(array(
        'description' => $des[$i],
        'observaciones' => $obs[$i],
        'eventos' => $eventos[$i],
