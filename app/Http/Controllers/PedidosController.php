...
public function index()
{
    ...
    $data2 = DB::table('tbl_pedidos') ->get();
    $data = DB::table('pedidos') ->get();
    return view('pedidos.index',compact('data','data2'));
}
...
public function consultaf(Request $request)
{
    ...
    $resultado= DB::select("SELECT * FROM pedidos WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mes'");
    $resultadoii= DB::select("SELECT * FROM pedidos WHERE DATE_FORMAT(fecha, '%Y-%m') = '$mesii'");
...
for($i = 0; $i<$inicio; $i++)
{
    $indicador = DB::table('pedidos')->insert(array(
        'description' => $des[$i],
        'observaciones' => $obs[$i],
        'eventos' => $eventos[$i],
